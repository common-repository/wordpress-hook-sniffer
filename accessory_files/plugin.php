 ...
 * THE WP HOOK SNIFFER PLUGIN. DO NOT USE THIS FILE IN
...
function do_action($tag, $arg = '') {
	...
	if ( is_array($wp_actions) ) // Build an array of action hooks as they are encountered
		$wp_actions[] = $tag;
	else
		$wp_actions = array($tag);

	$wp_current_filter[] = $tag; // Add action hook to temporary array; used by current_filter()
	...
	if ( !isset($wp_filter[$tag]) ) { // If action hook encountered does not have any action functions tied to it, then skip to next hook
		array_pop($wp_current_filter); // Remove reference to action hook that was just added to temporary array
	...
	// Sort
	if ( !isset( $merged_filters[ $tag ] ) ) { // If triggered action hook does not exist in the list of functions to be merged, then
		ksort($wp_filter[$tag]); // Sort the filter array
		$merged_filters[ $tag ] = true;
	}
